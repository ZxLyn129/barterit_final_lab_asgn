...
import 'package:barterit_app/interfaces/orderbuyer_screen.dart';
...
class MyPurchasesDetailsPage extends StatefulWidget {
  final User user;
  final OrderDetails order;

  const MyPurchasesDetailsPage(
      {Key? key, required this.order, required this.user})
      : super(key: key);

  @override
  State<MyPurchasesDetailsPage> createState() => _MyPurchasesDetailsPageState();
}

class _MyPurchasesDetailsPageState extends State<MyPurchasesDetailsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("Barter Details"),
      ),
                    ...
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          Text(
                            "Thank you for the Barter with Us",
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                          widget.order.sellerName.toString().toUpperCase(),
  ...
  void updateStatus() {
    ...
    if (widget.order.orderStatus.toString() == 'Shipping') {
      http.post(
        ...
        if (response.statusCode == 200) {
          ...
          if (jsondata['status'] == 'success') {
            ...
            setState(() {
              widget.order.orderStatus = 'Completed';
              widget.order.orderCompleteDate = date;
            });
            Navigator.pop(context);
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(
                builder: (content) => MyPurchsePage(
                  user: widget.user,
                ),
              ),
            );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("The item has not shipped")),
      );
    }
  }
}
