...
if (isset($_POST['buyerid'])){
	...
	$sqlorder = "SELECT * FROM `orders_tbl` JOIN `items_tbl` ON orders_tbl.item_id = items_tbl.itm_id JOIN `users_tbl` ON orders_tbl.seller_id = users_tbl.user_id WHERE orders_tbl.buyer_id = '$buyerid' ORDER BY orders_tbl.order_date DESC";
	...
	if ($result->num_rows > 0) {
		...
		while ($row = $result->fetch_assoc()) {
			...
			$orderdetails['seller_name'] = $row['user_name'];
